...
async function CheckContentUrl(url) {
  ...
  const results = await page.evaluate(() => {
    let hasMessenger = false;
    let hasBlogSection = false;
    let blogUrls = [];

    // Check for Facebook Messenger by looking for common classes or attributes
    if (
    ...
    // Check for links containing "blog"
    const links = Array.from(document.querySelectorAll("a[href]"));
    links.forEach((link) => {
      if (link.href.toLowerCase().includes("blog")) {
        blogUrls.push(link.href);
        hasBlogSection = true;
      }
    });
  ...
  return {
    url,
    hasMessenger: results.hasMessenger,
    hasBlogSection: results.hasBlogSection,
    blogUrls: results.blogUrls,
  };
}
